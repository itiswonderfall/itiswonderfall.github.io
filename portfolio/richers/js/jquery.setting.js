// JavaScript Document
$(function() {
	//var send=document.getElementById('send'),times=11,timer=null;
	var jj_times = 11;
	var jj_timer = null;
	$('#send').on('click',function(){
		var jj_sendClick = this;
		$(this).attr('disabled',true);
		jj_timer = setInterval(function(){
			jj_times --;
			jj_sendClick.value = jj_times + "秒後重試";
			if(jj_times <= 0){
				jj_sendClick.disabled =false;
				//this.attr('disabled',false);
				//alert(1212);
				jj_sendClick.value = "傳送驗證碼";
	...
	/*send.onclick=function(){
		...
		timer = setInterval(function(){
			...
			if(times <= 0){ that.disabled =false;
				that.value = "傳送驗證碼";
				clearInterval(timer);
				times = 11;
			};
